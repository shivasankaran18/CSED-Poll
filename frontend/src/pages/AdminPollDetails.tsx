...
export function AdminPollDetails() {
  const [pollData, setPollData] = useState(null);
  const [search,]=useSearchParams("id")
  useEffect(() => {
  ...
  return (
    <div className="container mx-auto p-4 max-w-full sm:max-w-4xl">
      ...
      <h1 className="text-2xl sm:text-3xl font-bold mb-4 text-blue-700 mt-6">
        {//@ts-ignore
        ...
        <TabsList className="w-full justify-start overflow-x-auto">
          ...
          pollData.polled.options.map((option) => (
            <TabsTrigger 
              ...
              className="whitespace-nowrap px-2 sm:px-4 data-[state=active]:bg-blue-100 data-[state=active]:text-blue-700"
            >
              {option.name}
            </TabsTrigger>
          ))}
